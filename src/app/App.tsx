...
const AppContent = () => {
  ...
  const [messageAccountct, setMessageAccountct] = useState(null);
  ...
  const handleSendMessage = (profileData) => {
    setMessageAccountct(profileData);
  ...
  return (
    <div className="relative">
      ...
      {currentScreen === 'directMessage' && (
        <DirectMessageScreen 
          onBack={navigateBack} 
          userName={messageAccountct?.name}
          userAvatar={messageAccountct?.name ? messageAccountct.name.split(' ').map(n => n[0]).join('').substring(0, 2) : null}
        />
      )}
